    ...
    {res?.cards[5]?.groupedCard?.cardGroupMap?.REGULAR?.cards.map((groupedCard) => (
      groupedCard?.card?.card?.itemCards?.map((itemCard) => (
        <li key={itemCard?.card?.info?.id}>{itemCard?.card?.info?.name}</li>
          ...
          <div className="upper_Portion" style={{display:"flex "}}>
          <div className="left"><img style={{width:"30ta0px",height:"200px"}} src={IMG_CDN_URL+res?.cards[0]?.card?.card?.info?.cloudinaryImageId} alt="" /></div>
         <div className="right">
          
         <h2 >{res?.cards[0]?.card?.card?.info?.name}</h2>
